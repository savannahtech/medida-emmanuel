...
class SceneReconstructionViewController: UIViewController, ARSessionDelegate {
    ...
    func configureSession() {
        // Ensure the device supports required ARKit features
        guard ARWorldTrackingConfiguration.supportsSceneReconstruction(.meshWithClassification) else {
            print("Scene reconstruction with mesh classification is not supported on this device.")
            showAlert(title: "Device Compatibility Issue",
                      message: "Scene reconstruction with mesh classification is not supported on this device."
            )
            return
        }
        
        guard ARWorldTrackingConfiguration.supportsFrameSemantics([.sceneDepth, .smoothedSceneDepth]) else {
            print("Scene depth or smoothed scene depth is not supported on this device.")
            showAlert(title: "Device Compatibility Issue",
                      message: "Scene depth or smoothed scene depth is not supported on this device."
            )
            return
        }
    ...
    override func didReceiveMemoryWarning() {
        ...
        showAlert(title: "Low Memory Warning",
                  message: "The recording has been paused. Do not quit the app until all files have been saved."
        )
    ...
    private func showAlert(title: String, message: String, actionTitle: String = "OK") {
        ...
        alert.addAction(UIAlertAction(
            title: NSLocalizedString(actionTitle, comment: "Default action"),
